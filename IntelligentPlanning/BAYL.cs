    ...
    public class BAYL : PTBase
    {
        ...
        public override string GetPlayMethodID(ConfigurationStatus.LotteryType pType, string playName)
        {
            ...
            if (AppInfo.Current.Lottery.Group == ConfigurationStatus.LotteryGroup.GPPK10)
            {
                ...
                if (playName == "猜前五复式")
                {
                    return "猜名次|猜前五名|cmc_mc5";
                }
                if (playName == "猜冠军猜冠军")
                {
                    return "猜名次|猜冠军|cmc_mc1";
                }
                if (playName.Contains("定位胆"))
                {
        ...
        {
            if (pType == ConfigurationStatus.LotteryType.NYFFC)
            {
                return "NYYFC";
            }
            if (pType == ConfigurationStatus.LotteryType.NY3FC)
            {
                return "NYSFC";
            }
            if (pType == ConfigurationStatus.LotteryType.NY5FC)
            {
                return "NYWFC";
            }
        ...
        public override void GetSite(ConfigurationStatus.LotteryType pType, string playName = "")
        {
            try
            {
                ...
                if (base.Prize == "")
                {
                    ...
                    base.Prize = CommFunc.GetIndexString(str, "\"customMo4\">", "~", 0);
                }
