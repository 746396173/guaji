namespace IntelligentPlanning
{
    ...
    public class SSHC : PTBase
    {
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.SSHCFFC)
            {
                return "6";
            }
            if (pType == ConfigurationStatus.LotteryType.SSHC3FC)
            {
                return "16";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.SSHCFF11X5)
            {
                return "14";
            }
            if (pType == ConfigurationStatus.LotteryType.SSHC3F11X5)
            {
                return "17";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.SSHCPK10)
            {
                return "20";
            }
            ...
            try
            {
                if (this.CheckLotteryIsVR(pType) && (base.Prize == ""))
                {
                    ...
                    if (pResponsetext != "")
                    {
                        ...
                        base.Prize = (1850.0 + ((Convert.ToDouble(str5) * 2.0) * 10.0)).ToString();
                    }
                }
            }
            catch
        ...
        public bool InputWeb(string pID, string pW, ref string pHint)
        {
            ...
            if (webVerifyCode != "")
            {
                ...
                HttpHelper.GetResponse(ref pResponsetext, pUrl, "POST", pData, loginLine, 0x2710, "UTF-8", true);
                flag = pResponsetext.Contains("交易协议");
                if (!flag)
                {
        ...
        {
            string pReferer = "";
            string pUrl = $"{this.GetVRLine()}/Account/LoginValidate?version=1.0&id=SS&data={pVRData}";
            string pResponsetext = "";
        ...
        public bool LoginWeb3()
        {
            ...
            bool flag = pResponsetext.Contains("盛世皇朝");
            if (flag)
            {
                base.Token = CommFunc.GetIndexString(pResponsetext, "value=\"", "\"", pResponsetext.IndexOf("__RequestVerificationToken"));
            }
