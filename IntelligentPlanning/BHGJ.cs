    ...
    public class BHGJ : PTBase
    {
        ...
        public override void GetAccountsMem(ConfigurationStatus.LotteryType pType, ConfigurationStatus.SCAccountData pInfo)
        {
            try
            {
                ...
                string pResponsetext = "";
                string pData = "action=find";
                HttpHelper.GetResponse(ref pResponsetext, accountsMemLine, "POST", pData, indexLine, 0x2710, "UTF-8", true);
                base.BankBalance = CommFunc.GetIndexString(pResponsetext, "\"UseMoney\":", ",", 0);
                AppInfo.Account.BankBalance = Convert.ToDouble(base.BankBalance);
                if ((base.Prize == "") && (pResponsetext != ""))
                {
                    base.Rebate = CommFunc.GetIndexString(pResponsetext, "\"HP\":", ",", 0);
        ...
        public override string GetAccountsMemLine(ConfigurationStatus.LotteryType pType) => 
            (this.GetLine() + "/api/user/login.ashx");
        ...
        public override string GetBetsLine(ConfigurationStatus.LotteryType pType) => 
            (this.GetLine() + "/api/bet/ssc_ajax.ashx");
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.BHGJFFC)
            {
                return "1008";
            }
            if (pType == ConfigurationStatus.LotteryType.BHGJ2FC)
            {
                return "1009";
            }
            if (pType == ConfigurationStatus.LotteryType.BHGJ5FC)
            {
                return "1006";
            }
            if (pType == ConfigurationStatus.LotteryType.BHGJXXLSSC)
            {
                return "1007";
            }
            if (pType == ConfigurationStatus.LotteryType.BHGJHGSSC)
            {
                return "1015";
            }
            if (pType == ConfigurationStatus.LotteryType.BHGJHN15C)
            {
                return "1016";
            }
            if (pType == ConfigurationStatus.LotteryType.BHGJHNFFC)
            {
                return "1017";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.BHGJPK10)
            {
                str = "1307";
            }
            return str;
        ...
        public override string GetIndexCode(ConfigurationStatus.LotteryType pType) => 
            $"{this.GetLine()}/api/bet/ssc_ajax.ashx?action=get_lottery_open&lottery_code={this.GetBetsLotteryID(pType)}&num={10}";
        ...
        public bool InputWeb(string pID, string pW, ref string pHint)
        {
            ...
            if (webVerifyCode != "")
            {
                ...
                string pUrl = this.GetLine() + "/api/user/login.ashx";
                string pResponsetext = "";
