namespace IntelligentPlanning
{
    ...
    public class QJC : PTBase
    {
        ...
        {
            base.PrizeDic.Clear();
            List<string> list = CommFunc.SplitString(CommFunc.GetIndexString(pResponseText, "\"userprize\":\"[", "]}]}]\"}", 0), "}]}]},", -1);
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.QJCFFC)
            {
                return "15";
            }
            if (pType == ConfigurationStatus.LotteryType.QJC2FC)
            {
                return "16";
            }
            if (pType == ConfigurationStatus.LotteryType.QJC45C)
            {
                return "45";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.QJC11X5)
            {
                return "18";
            }
            if (pType == ConfigurationStatus.LotteryType.QJCHL15C)
            {
                return "21";
            }
            if (pType == ConfigurationStatus.LotteryType.QJCAM15C)
            {
                return "22";
            }
            if (pType == ConfigurationStatus.LotteryType.QJCXNFFC)
            {
                return "19";
            }
            if (pType == ConfigurationStatus.LotteryType.QJCPDFFC)
            {
                return "20";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.QJCPK10)
            {
                str = "83";
            }
            return str;
        ...
        public override string GetLotteryLine(ConfigurationStatus.LotteryType pType, bool pAll = false) => 
            $"{this.GetLine()}/UserService.aspx?flag={this.GetPTLotteryName(pType)}Bean";
        ...
        public bool InputWeb(string pID, string pW, ref string pHint)
        {
            ...
            string pUrl = this.GetLine() + "/UserService.aspx";
            string pResponsetext = "";
            string str4 = HttpUtility.UrlEncode(pW);
            string pData = $"flag=LoginBean_loginnew&un={pID}&pwd={str4}";
        ...
        public override void QuitPT()
        {
            ...
            string indexLine = this.GetIndexLine();
            string pResponsetext = "";
            string pData = "flag=LoginBean_loginout";
