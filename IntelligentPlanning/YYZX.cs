namespace IntelligentPlanning
{
    ...
    public class YYZX : PTBase
    {
                ...
                foreach (string str3 in fNNumberDic.Keys)
                {
                    ...
                    foreach (string str4 in dictionary2.Keys)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                format = "balls%22%3A%7B%7D%2C%22bigCodes%22%3A%22{0}%22%7D%7D%2C%22";
                            }
                            else
                            {
                                format = "balls%22%3A%7B{0}%7D%7D%7D%2C%22";
                            }
                            format = string.Format(format, this.GetNumberList1(pTNumberList, plan.Play, null));
                            string str7 = "orderlist=%7B%22items%22%3A%7B%22{0}%22%3A%7B%22rulecode%22%3A%22{1}%22%2C%22total%22%3A%22{2}%22%2C%22itemcount%22%3A{3}%2C%22times%22%3A%22{4}%22%2C%22yjf%22%3A{5}%2C%22repoint%22%3A%22{6}%22%2C%22mode%22%3A{7}%2C%22{8}total%22%3A%22{9}%22%2C%22itemcount%22%3A{10}%2C%22ordercount%22%3A{11}%2C%22sumCount%22%3A{12}%2C%22lottery%22%3A%22{13}%22%2C%22currExpect%22%3A%22{14}%22%7D";
                            string str8 = string.Concat(new object[] { CommFunc.Random(0x3b9aca00, 0x77359400).ToString(), CommFunc.Random(100, 0x3e7), "-", CommFunc.MixRandom(8) });
                            str7 = string.Format(str7, new object[] { str8, this.GetPlayMethodID(plan.Type, plan.Play), plan.AutoTotalMoney(str4, true), num, Convert.ToInt32(plan.AutoTimes(str4, true)), num2, base.Rebate, base.Prize, format, plan.AutoTotalMoney(str4, true), num, 1, 1, this.GetPTLotteryName(plan.Type), this.GetBetsExpect(plan.CurrentExpect, "") });
                            HttpHelper.GetResponse(ref pResponsetext, betsLine, "POST", str7, lotteryLine, base.BetsTime3, "UTF-8", true);
            ...
            else if (AppInfo.Current.Lottery.Group == ConfigurationStatus.LotteryGroup.GPPK10)
            {
                ...
                else if (CommFunc.CheckPlayIsFS(playName))
                {
                    ...
                    for (num = 0; num < pNumberList.Count; num++)
                    {
                        ...
                        List<string> list7 = CommFunc.SplitString(str2, " ", -1);
                        foreach (string str8 in list7)
                        {
                            string str9 = $"{str8}";
                            pList.Add(str9);
                        }
        ...
        public bool LoginWeb()
        {
            ...
            bool flag = pResponsetext.Contains("易赢在线");
            return true;
        }
