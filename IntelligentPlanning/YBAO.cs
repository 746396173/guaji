namespace IntelligentPlanning
{
    ...
    public class YBAO : PTBase
    {
        ...
        public override bool CheckReturn(string pResponseText, bool pIsChange) => 
            ((pResponseText == "{\"msg\":\"\"}") || (pResponseText == "投注成功"));
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.YBAOFFC)
            {
                return "15";
            }
            if (pType == ConfigurationStatus.LotteryType.YBAO2FC)
            {
                return "16";
            }
            if (pType == ConfigurationStatus.LotteryType.YBAO45C)
            {
                return "45";
            }
        ...
        public override string GetIndexCode(ConfigurationStatus.LotteryType pType)
        {
            if (pType == ConfigurationStatus.LotteryType.PK10)
            {
                return (this.GetLine() + "/MobileService.aspx");
            }
            return $"{this.GetLine()}/page/YLFXssc.shtml?id={this.GetBetsLotteryID(pType)}";
        }
        ...
        public override string GetLotteryLine(ConfigurationStatus.LotteryType pType, bool pAll = false) => 
            $"{this.GetLine()}/lottery.shtml?{this.GetPTLotteryName(pType)}";
