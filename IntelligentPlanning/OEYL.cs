namespace IntelligentPlanning
{
    ...
    public class OEYL : PTBase
    {
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.OEFFC)
            {
                return "6";
            }
            if (pType == ConfigurationStatus.LotteryType.OE3FC)
            {
                return "16";
            }
        ...
        public override string GetWebVerifyCode(string pVerifyCodeFile)
        {
            ...
            try
            {
                ...
                goto Label_0092;
            Label_0051:
                HttpHelper.GetResponse10(ref pResponsetext, pUrl, "POST", pData, loginLine, 0x2710, "UTF-8", true);
                num++;
        ...
        public bool InputWeb(string pID, string pW, ref string pHint)
        {
            ...
            if (webVerifyCode != "")
            {
                ...
                HttpHelper.GetResponse(ref pResponsetext, pUrl, "POST", pData, loginLine, 0x2710, "UTF-8", true);
                flag = pResponsetext.Contains("彩票大厅") || pResponsetext.Contains("游戏大厅");
                if (!flag)
                {
        ...
        {
            string pReferer = "";
            string pUrl = $"{this.GetVRLine()}/Account/LoginValidate?version=1.0&id=OE&data={pVRData}";
            string pResponsetext = "";
        ...
        public bool LoginWeb3()
        {
            ...
            bool flag = pResponsetext.Contains("欧亿娱乐");
            if (flag)
            {
                base.Token = CommFunc.GetIndexString(pResponsetext, "value=\"", "\"", pResponsetext.IndexOf("__RequestVerificationToken"));
            }
