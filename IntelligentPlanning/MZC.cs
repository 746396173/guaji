namespace IntelligentPlanning
{
    ...
    public class MZC : PTBase
    {
        ...
        public override bool CheckReturn(string pResponseText, bool pIsChange) => 
            (pResponseText.Contains("\"State\":0") || (pResponseText == "投注成功"));
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.MZCFFC)
            {
                return "6";
            }
            if (pType == ConfigurationStatus.LotteryType.MZC3FC)
            {
                return "16";
            }
        ...
        public bool InputIDWeb(string pID, string pW, ref string pHint)
        {
            bool flag = false;
            ...
            if (webVerifyCode != "")
            {
                ...
                string pResponsetext = "";
                string pData = $"__RequestVerificationToken={base.Token}&loginid={pID}&CaptchaDeText={base.VerifyCodeToken}&CaptchaInputText={webVerifyCode}";
                HttpHelper.GetResponse(ref pResponsetext, pUrl, "POST", pData, loginLine, 0x2710, "UTF-8", true);
        ...
        public bool InputPWWeb(string pID, string pW, ref string pHint)
        {
            ...
            string pData = $"__RequestVerificationToken={base.Token}&LoginID={pID}&Password={str4}&Greetings=";
            HttpHelper.GetResponse(ref pResponsetext, loginLinePW, "POST", pData, loginLine, 0x2710, "UTF-8", true);
            bool flag = pResponsetext.Contains("交易协定");
            if (!flag)
            {
                pHint = CommFunc.GetIndexString(pResponsetext, "\"validation-summary-errors font-p\"><ul><li>", "<", 0);
            }
        ...
        {
            string pReferer = "";
            string pUrl = $"{this.GetVRLine()}/Account/LoginValidate?version=1.0&id=MZC&data={pVRData}";
            string pResponsetext = "";
            HttpHelper.GetResponse(ref pResponsetext, pUrl, "GET", string.Empty, pReferer, base.BetsTime1, "UTF-8", true);
            bool flag = pResponsetext.Contains("梦之城 VR");
            if (flag)
            {
                base.Token = CommFunc.GetIndexString(pResponsetext, "value=\"", "\"", pResponsetext.IndexOf("__RequestVerificationToken"));
            }
        ...
        public bool LoginWeb()
        {
            ...
            bool flag = pResponsetext.Contains("梦之城娱乐");
            if (flag)
            {
        ...
        public override bool WebLoginMain(string pID, string pW, ref string pHint)
        {
            ...
            if (!this.InputIDWeb(pID, pW, ref pHint))
            {
                return false;
            }
