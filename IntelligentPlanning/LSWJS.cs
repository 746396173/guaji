namespace IntelligentPlanning
{
    ...
    public class LSWJS : PTBase
    {
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.LSWJSFFC)
            {
                return "29";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJS5FC)
            {
                return "30";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJSOTXFFC)
            {
                return "30001";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJSMG15C)
            {
                return "20051";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJSMG45M)
            {
                return "20011";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJSOZFFC)
            {
                return "20031";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJSOZ35C)
            {
                return "21011";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.LSWJSHGSSC)
            {
                return "47";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJSDJSSC)
            {
                return "65";
            }
            if (pType == ConfigurationStatus.LotteryType.LSWJSTXFFC)
            {
                return "67";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.LSWJSFF11X5)
            {
                return "60";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.LSWJSFFPK10)
            {
                str = "40001";
            }
            return str;
        ...
        public override string GetLHPlayMethodID(string playName)
        {
            string str = "";
        ...
        public override string GetNumberList1(List<string> pNumberList, string playName, List<int> pRXWZ = null)
        {
            ...
            if (AppInfo.Current.Lottery.Group == ConfigurationStatus.LotteryGroup.GPSSC)
            {
                ...
                else if (CommFunc.CheckPlayIsLH(playName))
                {
                    ...
                    {
                        str2 = AppInfo.LHDic[pNumberList[num2]].ToString();
                        list.Add(str2);
                    }
                    str = $"{this.GetLHPlayMethodID(playName)}-{CommFunc.Join(list, "%2C")}";
                }
        ...
        public string GetWebVerifyCode1(string pID, string pVerifyCodeFile)
        {
            string pVerifyCode = "";
            try
            {
                ...
                {
                    return this.GetWebVerifyCode1(pID, pVerifyCodeFile);
                }
            }
            catch
            {
        ...
        {
            bool flag = false;
            string str = this.GetWebVerifyCode1(pID, AutoBetsWindow.VerifyCodeFile);
            if (str != "")
            {
                string loginLine = this.GetLoginLine();
                string pUrl = this.GetLine() + "/login";
                string pResponsetext = "";
                string str5 = HttpUtility.UrlEncode(pW);
                string pData = $"user_name={pID}&password={str5}&valid_code={str}";
        ...
        public bool LoginWeb()
        {
            ...
            return pResponsetext.Contains("拉斯维加斯");
        }
