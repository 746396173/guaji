namespace IntelligentPlanning
{
    ...
    public class LUDI : PTBase
    {
                    ...
                    foreach (string str4 in dictionary2.Keys)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                str7 = "{" + str7 + "}";
                                HttpHelper.GetResponse1(ref pResponsetext, betsLine, "POST", str7, lotteryLine, AppInfo.PTInfo.BetsTime2, "UTF-8", true);
                            }
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.LUDIFFC)
            {
                return "6";
            }
            if (pType == ConfigurationStatus.LotteryType.LUDI3FC)
            {
                return "16";
            }
        ...
        public override string GetLoginLinePW() => 
            (this.GetLine() + "/Account/LoginVerify");
        ...
        public bool InputWeb(string pID, string pW, ref string pHint)
        {
            ...
            if (webVerifyCode != "")
            {
                ...
                if (!flag)
                {
                    ...
                    pHint = CommFunc.GetIndexString(pResponsetext, "data-valmsg-replace=\"true\">", "<", 0);
                    return flag;
                }
        ...
        {
            string pReferer = "";
            string pUrl = $"{this.GetVRLine()}/Account/LoginValidate?version=1.0&id=LD&data={pVRData}";
            string pResponsetext = "";
            HttpHelper.GetResponse(ref pResponsetext, pUrl, "GET", string.Empty, pReferer, base.BetsTime1, "UTF-8", true);
            bool flag = pResponsetext.Contains("鹿鼎娱乐 VR");
            if (flag)
            {
                base.Token = CommFunc.GetIndexString(pResponsetext, "value=\"", "\"", pResponsetext.IndexOf("__RequestVerificationToken"));
            }
        ...
        {
            if ((AppInfo.App != ConfigurationStatus.AppType.OpenData) && !this.LoginWeb())
            {
                return false;
            }
            if (!this.GetLoginYZM())
