namespace IntelligentPlanning
{
    ...
    public class SKYYL : PTBase
    {
        ...
        public override bool CheckReturn(string pResponseText, bool pIsChange) => 
            (pResponseText.Contains("\"ErrorMessage\":null") || (pResponseText == "投注成功"));
        ...
        public override string GetBetsLotteryID(ConfigurationStatus.LotteryType pType)
        {
            ...
            if (pType == ConfigurationStatus.LotteryType.DB15C)
            {
                return "107";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.SKYFFC)
            {
                return "6";
            }
            if (pType == ConfigurationStatus.LotteryType.SKY2FC)
            {
                return "40";
            }
            if (pType == ConfigurationStatus.LotteryType.JFYLGG15C)
            {
                return "98";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.VRSXFFC)
            {
                return "34";
            }
            if (pType == ConfigurationStatus.LotteryType.VRTXFFC)
            {
                return "42";
            }
            if (pType == ConfigurationStatus.LotteryType.VRMXSC)
            {
                return "35";
            }
            if (pType == ConfigurationStatus.LotteryType.VRYYPK10)
            {
                return "37";
            }
            if (pType == ConfigurationStatus.LotteryType.VRZXC11X5)
            {
                return "38";
            }
            ...
            if (pType == ConfigurationStatus.LotteryType.SKY2F11X5)
            {
                return "39";
            }
        ...
        public bool InputWeb(string pID, string pW, ref string pHint)
        {
            ...
            if (webVerifyCode != "")
            {
                ...
                HttpHelper.GetResponse(ref pResponsetext, pUrl, "POST", pData, loginLine, 0x2710, "UTF-8", true);
                flag = pResponsetext.Contains("取款时间");
                if (!flag)
                {
                    pHint = pResponsetext;
                    ...
                    pHint = CommFunc.GetIndexString(pResponsetext, "validation-summary-errors font-p\"><ul><li>", "<", 0);
                    return flag;
                }
                base.Token = CommFunc.GetIndexString(pResponsetext, "value=\"", "\"", pResponsetext.IndexOf("__RequestVerificationToken"));
            }
        ...
        {
            string pReferer = "";
            string pUrl = $"{this.GetVRLine()}/Account/LoginValidate?version=1.0&id=SKY&data={pVRData}";
            string pResponsetext = "";
            HttpHelper.GetResponse(ref pResponsetext, pUrl, "GET", string.Empty, pReferer, base.BetsTime1, "UTF-8", true);
            bool flag = pResponsetext.Contains("SKY娱乐 VR") || pResponsetext.Contains("竞速娱乐 VR");
            if (flag)
            {
                base.Token = CommFunc.GetIndexString(pResponsetext, "value=\"", "\"", pResponsetext.IndexOf("__RequestVerificationToken"));
            }
        ...
        public bool LoginWeb()
        {
            ...
            bool flag = pResponsetext.Contains("SKY娱乐");
            if (flag)
            {
                base.Token = CommFunc.GetIndexString(pResponsetext, "value=\"", "\"", pResponsetext.IndexOf("__RequestVerificationToken"));
                base.VerifyCodeToken = CommFunc.GetIndexString(pResponsetext, "/DefaultCaptcha/Generate?t=", "\"", 0);
        ...
        public override void QuitPT()
        {
            ...
            string indexLine = this.GetIndexLine();
            string pResponsetext = "";
            string pData = $"__RequestVerificationToken={HttpUtility.UrlEncode(base.Token)}";
